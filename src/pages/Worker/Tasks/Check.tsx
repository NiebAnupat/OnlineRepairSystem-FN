...
export const Check = () => {
  ...
  const pendingCases: Case[] | null = useCaseStore(
    (state) => state.pendingCases
  );
  ...
  const search = () => {
    const filtered: Case[] | undefined = pendingCases?.filter((c) => {
      ...
      return (
        c.case_id == Number(searchQuery) ||
        c.name_case.includes(searchQuery) ||
        c.detail_case.includes(searchQuery) ||
        c.status_id.toString().includes(searchQuery) ||
        c.statuses.status_name.includes(searchQuery) ||
        c.place_case.includes(searchQuery) ||
                        ...
                        .map((item, index) => {
                          return (
                            <tr key={index}>
                              ...
                              <td style={{ textAlign: "center" }}>
                                {new Date(item.date_case).toLocaleDateString(
                                  "th-TH"
                                )}
                              </td>
                              <td style={{ textAlign: "center" }}>
                                {<StatusBadge status_id={item.status_id} />}
                              </td>
                              <td style={{ textAlign: "center" }}>
                                <Flex px={"xl"} justify={"space-around"}>
                                  ...
                                  >
                                    <ActionIcon
                                      ...
                                      color={"indigo.9"}
                                      onClick={() =>
                                        showDetail(item.case_id, imageHeight)
                                      }
                                    >
